...
public class ConsumerUndo {
    ...
    public String execute(ConsumerUndoReqBo reqBO) {
        ...
        //【撤销金额】，消费撤销时必须和原消费金额相同；单位：分
        data.put("txnAmt", reqBO.getTxnAmt());
        //交易币种(境内商户一般是156 人民币)
        ...
        Map<String, String> rspData = AcpService.post(reqData, reqUrl, UnionpayConstants.ENCODING);
            ...
            if (AcpService.validate(rspData, UnionpayConstants.ENCODING)) {
                ...
                if (code00.equals(respCode)) {
                    ...
                    System.out.println("交易已受理，等待接收通知成功 或 主动查询确定交易状态");
                        ...
                        "05".equals(respCode)) {
                    //后续需发起交易状态查询交易确定交易状态。
                    System.out.println("需手动查询确定交易状态");

                } else {
                    //其他应答码为失败请排查原因
                    System.out.println("交易失败请排查原因");

                }
            } else {
                LogUtil.writeErrorLog("验证签名失败");
                System.out.println("检查验证签名失败的原因");
            }
        } else {
            //未返回正确的http状态
            LogUtil.writeErrorLog("未获取到返回报文或返回http状态码非200");
        }
        String reqMessage = HtmlUtils.genHtmlResult(reqData);
        String rspMessage = HtmlUtils.genHtmlResult(rspData);
