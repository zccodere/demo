...
public class SdkConfig {
     ...
     * 配置文件中的单笔交易查询URL常量.
     */
    public static final String SDK_SINGLE_QUERY_URL = "acpsdk.singleQueryUrl";
    /**
     * 配置文件中的批量交易查询URL常量.
     */
    public static final String SDK_BATCH_QUERY_URL = "acpsdk.batchQueryUrl";
    /**
     * 配置文件中的批量交易URL常量.
     */
    public static final String SDK_BATCH_TRANS_URL = "acpsdk.batchTransUrl";
    /**
     * 配置文件中的文件类交易URL常量.
     */
    public static final String SDK_FILE_TRANS_URL = "acpsdk.fileTransUrl";
    /**
     * 配置文件中的有卡交易URL常量.
     */
    ...
    private void loadBase(Properties pro) {
        ...
        value = pro.getProperty(SDK_BATCH_QUERY_URL);
        if (!SdkUtil.isEmpty(value)) {
            this.batchQueryUrl = value.trim();
        }
        value = pro.getProperty(SDK_BATCH_TRANS_URL);
        if (!SdkUtil.isEmpty(value)) {
            this.batchTransUrl = value.trim();
        }
        value = pro.getProperty(SDK_FILE_TRANS_URL);
        if (!SdkUtil.isEmpty(value)) {
            this.fileTransUrl = value.trim();
        }
        value = pro.getProperty(SDK_SINGLE_QUERY_URL);
        if (!SdkUtil.isEmpty(value)) {
